...
export class StudentRepository {
  ...
  async findTasksByStudent(
    ...
    try {
      ...
      const validatedItemsPerPage = isNaN(Number(itemsPerPage))
        ? 10
        : Number(itemsPerPage);
  ...
  async getStudentDashboard(studentId: string) {
    try {
      ...
      const completedTask = await prisma.taskSubmission.count({
      ...
      const currentWeekTaskSubmission = await prisma.taskSubmission.count({
        where: {
          studentId,
          status: true,
          createdAt: {
      ...
      return {
        ...
        totalGradeThisWeek,
        currentWeekTaskSubmission,
      };
    } catch (error) {
      console.log(
        "Error occured while fetching dashboard details in student repository",
        error
      );
      return null;
    }
