import { IStudentTaskSubmission, ITask } from "../../interface/IStudent.js";
...
export default class StudentUseCase {
  ...
  async getTasksByStudent(
    ...
    try {
      ...
      const tasks = await this.studentRepo.findTasksByStudent(
        studentId,
        searchTerm,
        sortBy,
        page,
      ...
      return {
        ...
        totalTaskCount: tasks.totalTaskCount,
        completedCount: tasks.completedCount,
        pendingCount: tasks.pendingCount,
        dueSoonCount: tasks.dueSoonCount,
      };
    } catch (error) {
      console.log(
        "error occured while fetching data using student in usecase",
        error
      );
      return {
  ...
  async addTaskAnswer(
    ...
    try {
      ...
      console.log(
        "Error occure while addind tasks answers in student use case",
        error
      );
      return {
        status: StatusCode.InternalServerError,
  ...
  async getDashboardData(
    ...
    try {
      ...
      console.log(
        "Error occured while fetching student dashboard data from usecase",
        error
      );
      return {
        status: StatusCode.InternalServerError,
